...
void Trace::impulse(char* name, int steps) {
...
void Trace::calcNormIIR(int steps) {
	for(int i=0;i<steps;i++) {
		filter(0);
	}
	for(int i=0;i<steps;i++) {
...
void Trace::calcCoeffLowp(float t) {
	if (taps) {
		fprintf(stderr,"calcCoeffLowp: Only IIR\n");
		exit(1);
	}
	calcCoeffPoles(-t,0);
	resetBuffer();
 ...
 **/
void Trace::calcCoeffTrace(int nTaps, float tau,int normalise) {
	taps=nTaps;
	if (taps>0) {
		if (!bufferFIR) {
