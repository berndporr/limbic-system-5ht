...
Isolearning::Isolearning(int nInputs, 
	...
	nFilters=nFi;
	learningRate=0.01; // this should work initially
	thirdFactor=1.0;
	decay=0.0F;
	delta=0.0F;
	actualActivity=0.0F;
	...
	for(int j=0;j<nChannels;j++) {
		...
		for(int i=0;i<nFilters;i++) {
			weight[j][i]=1.0;
			setWeight(j,i,getWeight(j,i));
			//fprintf(stderr,"weights[%d][%d]= %f\n", j,i,getWeight(j,i));
		}
	}
...
void Isolearning::setReflexAsTraces(int nTaps,
					float tau,
					int n) {
	for(int i=0;i<1;i++) {
		for(int j=0;j<nFilters;j++) {
...
void Isolearning::prediction(int) {
  ...
  fprintf(stderr,"!!!weight: %f\n",weight[1][9]);
	...
	for(int j=1;j<nChannels;j++) {
		...
		for(int i=0;i<nFilters;i++) 
		  {
		    actualActivity=actualActivity+(getWeight(j,i)*getFilter(j,i)->getActualOutput());//getWeight(j,i)
		    fprintf(stderr,"weight: %f\n",actualActivity); 
		  }
	}
		...
		for(int i=0;i<nFilters;i++) 
		  {
		    ...
		    float correl=thirdFactor*delta*getFilter(j,i)->getActualOutput()-decay;//*weight[j][i]*fabs(getFilter(j,i)->getActualOutput()*fabs(delta));
			// calculate the integral by a linear approximation
			float integral=correl-(correl-lastCorrel[j][i])/2.0;
			// set the weight
			float wt=getWeight(j,i)+1*integral;
			//if (wt!=0)
			//fprintf(stderr,"wt=%f\n",wt);
			setWeight(j,i,getWeight(j,i)+learningRate*integral);
