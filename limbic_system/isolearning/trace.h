...
/**
 * "Memory" like structure: Either a trace with a decay constant or
 * a low pass filter.
 **/
...
class Trace {
 ...
 public:
	 ...
	 **/
	void calcCoeffTrace(int nTaps, float tau=0, int normalise=0);

	/**
	 * Calculates the coefficients of a second order lowpass. t is the time constant.
	 **/
	void calcCoeffLowp(float t);
	...
	/**
	 * Calculates the norm of the IIR filters
	 **/
	void calcNormIIR(int steps=1000);

	/**
	 * Generates an acsii file with the impulse response of the filter.
	 **/
	void impulse(char* name,int steps=1000);
