...
class Isolearning {
 ...
 public:
   ...
   **/
  void setLearningRate(float rate=0.00001) {learningRate=rate;};
 ...
 public:
   ...
   * get the derivate
   **/
  float getDerivative() {return delta;};
 public:
  /**
   *Get filter output
   **/

  float getFilterOutput(int j,int i) {
	  return getFilter(j,i)->getActualOutput();};

  //get both coponents of the correlation
 public:
  /**
   * get the AutoCorrelation
   **/
  float getAutoCorr(int j,int i) {
	  return deltaAutoCorr*getFilter(j,i)->getActualOutput();};

 public:
  /**
   * get the ICO correlation term
   **/

 float getICOCorr(int j,int i) {
	 return weightdeltaUnot*getFilter(j,i)->getActualOutput();};
 ...
 public:
  ...
  float getCorrel(int j,int i) {
	  return	lastCorrel[j][i];};
...
private:
 ...
 private:
  /**
   * The sum of the autocorrelation term 
   **/
  float SumAutoCorr;
 
 private:
  /**
   * The sumof the auto correlation one time step before
   **/
  float lastAutoCorr;

 private:
  /**
   *the difference between the autocorrelation terms
   **/
  float deltaAutoCorr;

 private:
  /**
   * The filter output term U0
   **/
  float Unot;

 private:
  /**
   * The derivative of the filter output term U0
   **/
  float deltaUnot;
 
 private:
  /**
   * The filter output term one time step before
   **/
  float lastUnot;

 private:
  /**
   * the weighted derivative of the output U0
   **/
  float weightdeltaUnot;
