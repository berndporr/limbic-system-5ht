	...
	for(int j=1;j<nChannels;j++) {
		...
		for(int i=0;i<nFilters;i++) {
			...
			float correl=thirdFactor*delta*getFilter(j,i)->getActualOutput()
				-decay*getWeight(j,i)*fabs(getFilter(j,i)->getActualOutput()*actualActivity);
				//				-decay*getWeight(j,i)*fabs(getFilter(j,i)->getActualOutput())*fabs(delta);
...
#ifdef NO_NEG_WEIGHTS
			if (getWeight(j,i)<0) {
				setWeight(j,i,0);
			}
#endif
#ifdef LIMIT_WEIGHTS
			if (getWeight(j,i) > LIMIT_WEIGHTS) {
				setWeight(j,i,LIMIT_WEIGHTS);
			}
#endif
